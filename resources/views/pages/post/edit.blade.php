            ...
            <div class="col-md-12 form-group">
                <div class="card card-primary">
                    ...
                    <div class="table-responsive">
                        <form id="form" action="{{ route('post.update', $data->id) }}" method="POST" enctype="multipart/form-data">
                            @csrf
                                ...
                                <div class="row">
                                    <div class="col-md-4">
                                        <div class="form-group">
                                            ...
                                            <select id="user" name="user_id" class="form-control" required>
                                                ...
                                                @foreach ($user as $row)
                                                <option value="{{ $row->id }}" <?= $row->id == $data->user_id ? 'selected' : ''; ?>>
                                                    {{ $row->name }}
                                                </option>
                                                @endforeach
                                            </select>
                                            ...
                                            <select id="category" name="category_id" class="form-control" required>
                                                ...
                                                <option value="{{ $row->id }}" <?= $row->id == $data->sub_category_id ? 'selected' : ''; ?>>
                                                    {{ $row->name }}
                                                </option>
                                                @endforeach
                                            </select>
                                            ...
                                            <select id="tags" name="tags[]" class="form-control" multiple required>
                                                @foreach ($tag as $row)
                                                <option value="{{ $row->id }}"
                                                    {{ in_array($row->id, $data->tags->pluck('id')->toArray()) ? 'selected' : '' }}>
                                                    {{ $row->name }}
                                                </option>
                                                @endforeach
                                            </select>
                                            ...
                                            @if ($data->thumbnail)
                                            <img id="modal-foto" src="{{ asset('dist/img/thumbnail/'. $data->thumbnail) }}" alt="Thumbnail" class="img-fluid">
                                            @else
                                    ...
                                    <div class="col-md-8 col-12">
                                        <div class="form-group">
                                            ...
                                            <textarea id="content" type="text" class="form-control" name="content">{!! $data->content !!}</textarea>
                                        </div>
                                    </div>
                                </div>
